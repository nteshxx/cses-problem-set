...
public class GridPaths {
    ...
    private static void traverseGrid(boolean[][] grid, int row, int column, int steps) {
        ...
        if ((grid[row][column])
                ...
                || ((row == 0 || row == 6) && column > 0 && column < 6 && !grid[row][column - 1]
                        && !grid[row][column + 1]) // UP DOWN WALLS
                || ((column == 0 || column == 6) && row > 0 && row < 6 && !grid[row - 1][column]
                        && !grid[row + 1][column]) // LEFT RIGHT
                // optimization 4
                // hit a block where unvisited blocks on both side on block on forward path
                || (row > 0 && row < 6 && column > 0 && column < 6 && grid[row - 1][column] && grid[row + 1][column]
                        && !grid[row][column - 1] && !grid[row][column + 1]) // UP DOWN
                || (row > 0 && row < 6 && column > 0 && column < 6 && grid[row][column - 1] && grid[row][column + 1]
                        && !grid[row - 1][column] && !grid[row + 1][column])) // LEFT RIGHT
        {
            return;
        }
